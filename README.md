...
An external signal is fed into the ERU unit using a GPIO pin. A service request interrupt is generated based on the falling edge of the external signal. An LED is toggled inside the ERU interrupt service routine (ISR).
...
3. Confirm the operation of the example:
   ...
   1. If using the XMC1400 Boot Kit, confirm that the User LED toggles when the Knob of the potentiometer(on P2.5) is turned backward and forward.
      See the [XMC1400 Boot kit Board User Manual](https://www.infineon.com/dgdl/Infineon-Board_Users_Manual_XMC1400_Boot_Kit.pdf-UM-v01_00-EN.pdf?fileId=5546d462525dbac401527815f9a073fd) for details if the potentiometer connections.

   2. If using the XMC4700 Relax Kit, connect a jumper wire at P1.15 and 3V3 of connector X2 of the evaluation kit. Confirm that the User LED toggles when a connection is made and broken between 3V3 and the P1.15 port pin.
...
The main loop waits for the occurrence of an external signal; with falling edge detection, an interrupt is generated by the ERU and the User LED is toggled inside the interrupt service routine.
...
Document Title: *CE232792* - *XMC MCU: ERU EXTINT*
